...
function mount_vss(){
      ...
      [ "$(ls -A /tmp/shadow/vss1)" ] && makegreen "Success! VSCs mounted on /tmp/shadow" || echo "No Volume Shadow Copies mounted"
}
...
makegreen "Mount disk images (E01, vmdk, vhd(x),raw, vss)"
echo "USAGE: ermount.sh -s -u -h 
	OPTIONS:
           -s ermount status
      ...
      Requirements: ewf-tools, afflib3, qemu-utils, mount
      Warning: forcefully disconnects mounted drives and NBD
...
# Setup mount directories and process any cli parameters
